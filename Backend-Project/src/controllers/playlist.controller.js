...
const getAllPlaylistOfAUser = asyncHandler(async(req, res) => {
    const user = req.user
    ...
    const playlists = await Playlist.aggregate([
        {$match: {owner:new  mongoose.Types.ObjectId(user._id)}},
        {
            ...
            $addFields: {
              firstThumbnail: {
                $cond: {
                  ...
                  then: { $arrayElemAt: ["$videoDetails.thumbnail", 0] },
                  else: null
                }
              }
            }
          ...
          {
            $project: {
              videoDetails: 0 // Exclude the detailed video information if not needed
            }
          }
    ])
   ...
   if(!playlists){
    return res.status(200).json(
        new ApiResponse(200, false, "No playlist found")
    )
   }
   return res.status(200).json(
